...
test "it fetches config properties from the OS environment" {
    ...
    const loadedCfg = try config.loadAsStruct(&[_][]const u8{}, .{ .unmask = true });
...
test "it fetches config properties from the OS environment and dotenv file" {
    ...
    const loadedCfg = try config.loadAsStruct(&[_][]const u8{".env.test"}, .{ .unmask = true });
    try std.testing.expect(loadedCfg.ZEBRA_TEST_OS_PORT == 3000);
    try std.testing.expectEqualStrings("secret_123", loadedCfg.ZEBRA_TEST_OS_API_KEY);
    try std.testing.expect(loadedCfg.ZEBRA_TEST_OS_IS_DEBUG == true);
    try std.testing.expectEqualStrings("testing", loadedCfg.ZEBRA_TEST_OS_NODE_ENV);
    try std.testing.expectEqualStrings("https://api.example.com/v1?query=test", loadedCfg.URL);
}

test "it loads a dotenv file as a map with all edge cases" {
    const allocator = std.testing.allocator;
    var cfg = try zebra.core.loadAsMap(allocator, &[_][]const u8{".env.test"}, .{ .unmask = true });
...
test "it loads a toml file as a map with all edge cases" {
    const allocator = std.testing.allocator;
    var cfg = try zebra.core.loadAsMap(allocator, &[_][]const u8{"env_test.toml"}, .{ .unmask = true });
    defer zebra.cleanup.deinitMap(allocator, &cfg);
    ...
    // 6. some credentials
    try std.testing.expectEqualStrings("bLablABlaFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY", cfg.get("AWS_SECRET_ACCESS_KEY").?);
    try std.testing.expectEqualStrings("bLablABlaFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY", cfg.get("AZURE_CLIENT_SECRET").?);
    try std.testing.expectEqualStrings("blablablasalt", cfg.get("PASSWORD_SALT").?);
    try std.testing.expectEqualStrings("token123", cfg.get("BITBUCKET_TOKEN").?);
...
test "it loads a yaml file as a map with all edge cases" {
    ...
    var cfg: std.StringHashMap([]u8) = try zebra.core.loadAsMap(allocator, &[_][]const u8{"env_test.yaml"}, .{ .unmask = true });
    defer zebra.cleanup.deinitMap(allocator, &cfg);
...
test "it fetches config masked properties from OS environment by default" {
    const allocator = std.testing.allocator;
    ...
    try std.testing.expect(loadedCfg.ZEBRA_TEST_OS_PORT == 3000);
    try std.testing.expectEqualStrings("XXXX", loadedCfg.ZEBRA_TEST_OS_API_KEY);
    try std.testing.expect(loadedCfg.ZEBRA_TEST_OS_IS_DEBUG == true);
    try std.testing.expectEqualStrings("testing", loadedCfg.ZEBRA_TEST_OS_NODE_ENV);
...
test "it loads multiple env files in a map with sensitive creds masked by default" {
    const allocator = std.testing.allocator;

    var cfg: std.StringHashMap([]u8) = try zebra.core.loadAsMap(allocator, &[_][]const u8{ "env_test.yaml", "env_test.toml" }, .{ .unmask = false });
    defer zebra.cleanup.deinitMap(allocator, &cfg);

    // ### yaml file tests below
    ...
    // 6. empty/null values (parents are not stored as values, only leaves)
    try std.testing.expect(cfg.get("empty_key") == null);
    try std.testing.expectEqualStrings("XXXX", cfg.get("next_key").?);
    ...
    // dots inside quotes (should not be split by the parser)
    try std.testing.expectEqualStrings("XXXX", cfg.get("quoted.key.with.dots").?);

    // special characters (urls and symbols)
    try std.testing.expectEqualStrings("https://ziglang.org", cfg.get("url_test").?);

    // end value
    try std.testing.expectEqualStrings("XXXX", cfg.get("final_key").?);

    // ### toml file tests below ###

    // 1. complex & dotted keys
    try std.testing.expectEqualStrings("XXXX", cfg.get("key").?);
    try std.testing.expectEqualStrings("UTF-8", cfg.get("character.encoding").?);
    // Note: Quoted keys usually retain quotes in the key string to avoid ambiguity
    if (cfg.get("\"127.0.0.1\"")) |val| {
        try std.testing.expectEqualStrings("localhost", val);
    ...
    // 6. some credentials
    try std.testing.expectEqualStrings("XXXX", cfg.get("AWS_SECRET_ACCESS_KEY").?);
    try std.testing.expectEqualStrings("XXXX", cfg.get("AZURE_CLIENT_SECRET").?);
    try std.testing.expectEqualStrings("XXXX", cfg.get("PASSWORD_SALT").?);
    try std.testing.expectEqualStrings("XXXX", cfg.get("BITBUCKET_TOKEN").?);
